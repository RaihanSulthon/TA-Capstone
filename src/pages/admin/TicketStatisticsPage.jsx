    ...
    <div className="max-w-7xl mx-auto px-3 md:px-4 py-4 md:py-6 overflow-x-hidden w-full">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 md:gap-4 mb-4 md:mb-6 overflow-hidden w-full">
        ...
        <div className="bg-white p-3 md:p-4 rounded-lg shadow-md min-w-0">
          ...
          <p className="text-lg md:text-2xl font-bold text-green-600">{userCounts.students}</p>
        </div>
        <div className="bg-white p-3 md:p-4 rounded-lg shadow-md min-w-0">
          <p className="text-xs md:text-sm text-gray-500 truncate">Admins</p>
          <p className="text-lg md:text-2xl font-bold text-red-600">{userCounts.admins}</p>
        </div>
        <div className="bg-white p-3 md:p-4 rounded-lg shadow-md min-w-0">
          <p className="text-xs md:text-sm text-gray-500 truncate">Total Laporan</p>
            ...
            {loading ? (
              ...
              currentUsers.map((user) => {
                ...
                const getRoleBadge = (role) => {
                  switch(role) {
                    case 'admin':
                      return <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800">Admin</span>;
                    case 'student':
                      return <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">Student</span>;
                    default:
                ...
                const getStatusSummary = (stats) => {
                  if (stats.total === 0) {
                    return <span className="text-gray-500 text-sm">Belum ada laporan</span>;
                  }
                    ...
                    <div className="space-y-1">
                      {stats.new > 0 && (
                        <div className="flex justify-between">
                          <span className="text-xs text-blue-600">{stats.new} Baru</span>
                        </div>
                      )}
                      {stats.in_progress > 0 && (
                        <div className="flex justify-between">
                          <span className="text-xs text-yellow-600">{stats.in_progress} Proses</span>
                        </div>
                      )}
                      {stats.done > 0 && (
                        <div className="flex justify-between">
                          <span className="text-xs text-green-600">{stats.done} Selesai</span>
                        </div>
                      )}
                    </div>
                  );
                ...
                const formatLastSubmission = (lastSubmission) => {
                  ...
                  try {
                    const date = lastSubmission.toDate ? lastSubmission.toDate() : new Date(lastSubmission);
                    return date.toLocaleDateString('id-ID', {
                      day: '2-digit',
                      month: 'short',
                      year: 'numeric'
                ...
                return (
                  <div key={user.id} className="p-3 hover:bg-gray-50">
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-3">
                      ...
                      <div className="bg-gray-50 p-2 rounded">
                        <div className="text-xs text-gray-600">Status Laporan</div>
                        <div className="mt-1">
                          {getStatusSummary(stats)}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="border-t border-gray-200 pt-2">
                      <div className="flex justify-between items-center">
                        <span className="text-xs text-gray-600">Terakhir Submit:</span>
                    ...
                    <tr key={user.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {stats.total === 0 ? (
                          ...
                          <div className="space-y-1">
                            {stats.new > 0 && <div className="text-blue-600">{stats.new} Baru</div>}
                            {stats.in_progress > 0 && <div className="text-yellow-600">{stats.in_progress} Proses</div>}
                            {stats.done > 0 && <div className="text-green-600">{stats.done} Selesai</div>}
                          </div>
                        )}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {stats.lastSubmission ? (
                          ...
                          <span className="text-gray-400">Belum pernah submit</span>
                        )}
                      </td>
                    </tr>
                  );
      ...
      <div className="mt-4 md:mt-6 bg-white rounded-lg shadow-md p-4 md:p-6">
        <h3 className="text-base md:text-lg font-medium text-gray-900 mb-3 md:mb-4">Summary Information</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 text-sm">
          <div className="bg-gray-50 p-3 rounded-md">
            <p className="text-gray-600">
              <span className="font-medium text-xs md:text-sm">Users with Activity:</span>
          ...
          <div className="bg-gray-50 p-3 rounded-md sm:col-span-2 lg:col-span-1">
            <p className="text-gray-600">
              <span className="font-medium text-xs md:text-sm">Total System Tickets:</span>
              <br className="sm:hidden" />
              <span className="text-purple-600 font-semibold"> {tickets.length}</span>
            </p>
          </div>
        </div>
      </div>
